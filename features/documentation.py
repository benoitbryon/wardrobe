...
@step(u'When I run ``make documentation``')
def when_i_run_make_documentation(step):
    """Build documentation."""
    ...
    # sphinx-build echoes 'makine output directory\n' to STDERR. Ignore it.
...
@step(u'When I run ``make readme``')
def when_i_run_make_readme(step):
    """Build README."""
    command = ['make', 'readme']
    process = subprocess.Popen(command, stdout=subprocess.PIPE,
                               stderr=subprocess.PIPE)
...
@step(u'And I ``docs/_build/html/README.html`` file is generated')
